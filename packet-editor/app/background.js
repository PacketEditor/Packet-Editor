...
var readFromSocket = function(socketId) {
  ...
  socket.read(socketId, function(readInfo) {
    ...
    if (content.length == 0) {
      ...
      var lengthLoc = headers.indexOf("Content-Length: ");
      var lengthEnd = data.indexOf("\r\n", lengthLoc);
      totalContentLength = parseInt(
          data.substring(lengthLoc + 15, lengthEnd), 10);
      readChunk();
    }
  });
...
socket.create("tcp", {}, function(_socketInfo) {
  ...
  try {
    socket.listen(socketInfo.socketId, "127.0.0.1", 36021, 20,
      function(result) {
    ...
    uiWindow.contentWindow.document.body.innerHTML =
        '<h1>Failed to start: port 36021 is in use.</h1>';
  }
});
